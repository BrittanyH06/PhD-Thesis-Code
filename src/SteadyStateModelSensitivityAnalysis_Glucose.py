...
#   - N_vec: pulse lengths analyzed (default: [2, 4, 5, 6, 8, 10])
...
def update_derived_params(p):
    """
    Given a parameter dictionary p with keys r_min, r_max, kappa_t, and lambda_0_glu,
    compute and add derived parameters:
      - delta_r: r_max - r_min
      - lambda_max: kappa_t * delta_r
      - kappa_n: 1 / (delta_r * (1/lambda_0_glu - 1/(kappa_t*delta_r)))
    """
    p = p.copy()
    p["delta_r"] = p["r_max"] - p["r_min"]
    p["lambda_max"] = p["kappa_t"] * p["delta_r"]
    p["kappa_n"] = 1.0 / (p["delta_r"] * ((1.0 / p["lambda_0_glu"]) - (1.0 / (p["kappa_t"] * p["delta_r"]))))
    return p
...
def compute_IC50(p):
    ...
    New IC50 formula:
      ...
      - IC50 = 0.5 * IC50_star * (lambda_0_glu/lambda_0_star + lambda_0_star/lambda_0_glu)
    """
    K_D = p["k_off"] / p["k_on"]
    lambda_0_glu = p["lambda_0_glu"]
    delta_r = p["delta_r"]
    kappa_t = p["kappa_t"]
    P_in = p["P_in"]
    P_out = p["P_out"]
    ...
    return 0.5 * IC50_star * ((lambda_0_glu / lambda_0_star) + (lambda_0_star / lambda_0_glu))
...
def model_equations(t, y, p, IC50_val, N):
    ...
    kappa_t, lambda_0_glu = p["kappa_t"], p["lambda_0_glu"]
    delta_r, kappa_n = p["delta_r"], p["kappa_n"]

    if t <= 1:
        a_ex = 0
        x = (r_u - r_min) * kappa_t
        s = x * (r_max - ((x * delta_r) * ((1.0 / lambda_0_glu) - (1.0 / (kappa_t * delta_r)))))
    elif t <= N + 1:
        a_ex = (4 * IC50_val) / N
        x = (r_u - r_min) * kappa_t
        s = x * (r_max - ((x * delta_r) * ((1.0 / lambda_0_glu) - (1.0 / (kappa_t * delta_r)))))
...
# ----------------------------------------------------------------
N_vec = [2, 4, 5, 6, 8, 10]
y0 = [0, params["r_min"] + (params["lambda_0_glu"] / params["kappa_t"]), 0]
...
def calculate_baseline_output(p):
    ...
    delay_dict = {}
    for N in N_vec:
        tMax = (N + 1) + 4 + (30.0 / p["lambda_0_glu"])
        ...
        pre, post = OD[1], OD[-1]
        delay_dict[N] = ((pre - post) / p["lambda_0_glu"]) + (time[-1] - time[1])
    
    max_delay = max(delay_dict.values())
    
    # Compute peak RNA/P for N = 4.
    N_val = 4
    tMax4 = (N_val + 1) + 4 + (30.0 / p["lambda_0_glu"])
